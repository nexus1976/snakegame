...
document.addEventListener('keydown', (e) => {
	if (e === undefined || e.key === undefined) return;
	let key = e.key;
...
const init = () => {
	...
	document.score = 0;
	document.level = 1;
	if(typeof document.game_loop !== undefined) clearInterval(document.game_loop);
...
const paint = () => {
	...
	if(nx == -1 || nx == document.w/document.cw
		|| ny == -1 || ny == document.h/document.cw
		|| check_collision(nx, ny, document.snake_array)) {
	...
	if(nx == document.food.x && ny == document.food.y) {
		...
		tail.x = nx; tail.y = ny;                                       	                           	            	tail.x = nx; tail.y = ny;
	}
